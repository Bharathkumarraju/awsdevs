...
9. In the first case when a message gets delivered may be we have some kind of customer reminder service(A lambda function) - this lambda fucntion delvers message(via Email) to customer saying that Thank you for ordering we appreciate your business

10. And completely separately there is two other subsystems(two SQS Queue's)' here they care about this same event but they do very different things so we dont want to mix the responsibilities of these different things.

11. We want to have a separate systems that allows us to process these events in parallel and completely independent of one another.
